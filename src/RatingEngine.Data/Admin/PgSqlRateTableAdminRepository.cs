...
public sealed class PgSqlRateTableAdminRepository : IRateTableAdminRepository
{
    private readonly DbConnectionFactory _db;
    public PgSqlRateTableAdminRepository(DbConnectionFactory db) => _db = db;
    ...
    public async Task<IReadOnlyList<RateTableSummary>> ListAsync(int coverageConfigId, CancellationToken cancellationToken = default)
    {
        ...
        return (await conn.QueryAsync<RateTableSummary>(new CommandDefinition(
            sql, new { CoverageConfigId = coverageConfigId }, cancellationToken: cancellationToken))).AsList();
    }
    ...
    public async Task<RateTableDetail?> GetAsync(int coverageConfigId, string name, CancellationToken cancellationToken = default)
    {
        ...
        var row = await conn.QueryFirstOrDefaultAsync<TableRow>(new CommandDefinition(
            tableSql, new { CoverageConfigId = coverageConfigId, Name = name }, cancellationToken: cancellationToken));
        if (row is null) return null;

        var colDefs = (await conn.QueryAsync<ColumnDefDetail>(new CommandDefinition(
            colDefSql, new { TableId = row.Id }, cancellationToken: cancellationToken))).AsList();
        ...
        return new RateTableDetail(
            ...
            row.EffStart, row.ExpireAt, row.CreatedAt, row.CreatedBy, colDefs);
    }
    ...
    public async Task<int> CreateAsync(CreateRateTableRequest req, string? actor = null, CancellationToken cancellationToken = default)
    {
        ...
        var id = await conn.QuerySingleAsync<int>(new CommandDefinition(insertSql, new
        {
    ...
    public async Task<long> AddRowAsync(int coverageConfigId, string tableName, CreateRateTableRowRequest req, CancellationToken cancellationToken = default)
    {
        ...
        var tableId = await conn.ExecuteScalarAsync<int?>(new CommandDefinition(
            tableIdSql, new { CoverageConfigId = coverageConfigId, Name = tableName }, cancellationToken: cancellationToken))
            ?? throw new InvalidOperationException($"Rate table '{tableName}' not found in coverage {coverageConfigId}.");

        return await conn.QuerySingleAsync<long>(new CommandDefinition(insertSql, new
        {
            RateTableId = tableId,
    ...
    public async Task<bool> UpdateRowAsync(int coverageConfigId, string tableName, long rowId, CreateRateTableRowRequest req, CancellationToken cancellationToken = default)
    {
        ...
        return await conn.ExecuteAsync(new CommandDefinition(sql, new
        {
            Id = rowId, CoverageConfigId = coverageConfigId, TableName = tableName,
            req.Key1, req.Key2, req.Key3, req.Key4, req.Key5,
            req.RangeFrom, req.RangeTo, req.Factor,
            req.AdditionalUnit, req.AdditionalRate, req.EffStart, req.ExpireAt
        }, cancellationToken: cancellationToken)) > 0;
    ...
    public async Task<bool> ExpireRowAsync(int coverageConfigId, string tableName, long rowId, DateOnly expireAt, CancellationToken cancellationToken = default)
    {
        using var conn = _db.Create();
        return await conn.ExecuteAsync(new CommandDefinition(
            """
            UPDATE "RateTableRow" SET "ExpireAt" = @ExpireAt 
            WHERE "Id" = @Id
              AND "RateTableId" = (SELECT "Id" FROM "RateTable" WHERE "CoverageConfigId" = @CoverageConfigId AND "Name" = @TableName)
            """,
            new { Id = rowId, ExpireAt = expireAt, CoverageConfigId = coverageConfigId, TableName = tableName }, 
            cancellationToken: cancellationToken)) > 0;
    }

    public async Task<bool> DeleteRowAsync(int coverageConfigId, string tableName, long rowId, CancellationToken cancellationToken = default)
    {
        using var conn = _db.Create();
        return await conn.ExecuteAsync(new CommandDefinition(
            """
            DELETE FROM "RateTableRow" 
            WHERE "Id" = @Id
              AND "RateTableId" = (SELECT "Id" FROM "RateTable" WHERE "CoverageConfigId" = @CoverageConfigId AND "Name" = @TableName)
            """,
            new { Id = rowId, CoverageConfigId = coverageConfigId, TableName = tableName }, 
            cancellationToken: cancellationToken)) > 0;
    }
